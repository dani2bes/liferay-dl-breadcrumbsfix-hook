...
<!-- start: my changes -->
<jsp:declaration>
		public String sanitizeFolderBreadcrumbs(String fBreadcrumbs) {
			String[] splitBreadcrumbs = StringUtil.split(fBreadcrumbs,StringPool.RAQUO);
			return StringUtil.merge(splitBreadcrumbs," \u00BB ");
		};
</jsp:declaration>
<!-- end: my changes -->
<%
		...
		>
			...
			<c:choose>
				<c:when test="<%= fileEntry != null %>">
					...
					<!-- start: my changes -->
					<jsp:scriptlet>
					  String rawBreadcrumbs = DLUtil.getAbsolutePath(renderRequest, fileEntry.getFolderId());
					  String folderBreadcrumbs = sanitizeFolderBreadcrumbs(rawBreadcrumbs);
					</jsp:scriptlet>
					<!-- end: my changes -->

					<liferay-ui:app-view-search-entry
						actionJsp='<%= (showActions) ? "/html/portlet/document_library/file_entry_action.jsp" : StringPool.BLANK %>'
						containerName="<%= folderBreadcrumbs %>"
				...
				<c:when test="<%= folder != null %>">
					...
					<!-- start: my changes -->
					<jsp:scriptlet>
					  String rawBreadcrumbs = DLUtil.getAbsolutePath(renderRequest, folder.getParentFolderId());
					  String folderBreadcrumbs = sanitizeFolderBreadcrumbs(rawBreadcrumbs);
					</jsp:scriptlet>
					<!-- end: my changes -->
